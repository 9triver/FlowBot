...
* {workbook} Set Cell {row} {column} {value}: 设置单元格的值
    ...
    * ```
        {workbook}.write_cell(row={row}, column={column}, value={value})
        ```
...
- check {str} is a phone number: 判断字符串是否为手机号
...
- Set {var} to the length of {strValue}
...
- Set {var} to substring {from} {to} to {strValue} 
